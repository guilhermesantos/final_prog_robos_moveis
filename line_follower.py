...
def setup_pins(config_dict):
	#GPIO.PWM(config_dict['motor1'], 1000)
	#GPIO.PWM(config_dict['motor2'], 1000)
	#GPIO.PWM(config_dict['motor3'], 1000)
	#GPIO.PWM(config_dict['motor4'], 1000)
	return None
...
def process_capture(capture, show_frame):
	while(capture.isOpened()):
		...
		y_lowest_midpoint = line_midpoints[line_midpoints != -1].shape[0]
		x_lowest_midpoint = int(line_midpoints[y_lowest_midpoint-1])
		frame[segmented_image == 0, 0] = 0
		frame[segmented_image == 0, 1] = 0
		frame[segmented_image == 0, 2] = 255
		...
		if(x_lowest_midpoint < image_width//2):
			print('virar para a esquerda')
		else:
			print('virar para a direita')
		#edge_frame = cv2.Canny(thresh_frame, 60, 200)
		#line_frame = apply_hough_transform(edge_frame)
		#kp_frame = detect_blobs(frame)

		'''contours, hierarchy = cv2.findContours(thresh_frame.copy(), 1, cv2.CHAIN_APPROX_NONE)
		if(len(contours) > 0):
